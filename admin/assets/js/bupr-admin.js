...
jQuery(document).ready(
	function ($) {
		...
		*----------------------------------------------------------*/
			...
			function () {
				...
				jQuery.post(
					bupr_admin_ajax_object.ajaxurl,
					{
						...
						'review_id': review_id,
						'nonce' : bupr_admin_ajax_object.nonce,
					},
					function (response) {
						if (response.data == 'review-approved-successfully') {
		...
		jQuery(document).on(
			'keyup',
			'#bupr_member_tab_title',
			function () {
				var value = jQuery(this).val();
				var matches = value.match(/\d+/g);
				if (matches != null) {
					$(this).next().text(bupr_admin_ajax_object.number_validation);
					$(this).parent().append('<span class="wbcom-field-error"></span>');
					$('#submit').attr("disabled",'disabled');
				}else{
					$(this).siblings('wbcom-field-error').remove();
					$(this).next().text('');
...
function GetDynamicTextBox(value) {
	...
	return '<div id="bupr-admin-row-criteria" class="bupr-admin-row border bupr-criteria"><div class="bupr-admin-col-6"><input name="bupr_admin_settings[rating_field_name][' + rand_num + ']" class = "buprDynamicTextBox" type="text" value = "' + value + '" placeholder="Add Review criteria eg. Member Response. " /></div>' +
